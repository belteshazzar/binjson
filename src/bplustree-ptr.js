/**
 * BPlusTreePtr - B+ tree variant that stores values by pointer reference
 * 
 * This implementation stores value objects separately and uses Pointers in the tree nodes.
 * During tree operations (splits, merges), only 8-byte pointers are copied instead of
 * potentially large value objects, which should improve performance for large values.
 * 
 * Usage pattern:
 *   const tree = new BPlusTreePtr('tree.bj');
 *   await tree.open();
...
/**
 * Node for persistent storage (pointer-based)
 * @private
 */
class NodeDataPtr {
    /**
     * Creates a node data object for serialization with pointer-based values
     * @param {number} id - Unique node ID
     * @param {boolean} isLeaf - Leaf flag
     * @param {Array} keys - Key array
     * @param {Array<Pointer>} valuePointers - Pointer array to values (leaf nodes)
     * @param {Array<Pointer>} children - Child pointers (internal nodes)
     * @param {Pointer} next - Pointer to next leaf
     */
    constructor(id, isLeaf, keys, valuePointers, children, next) {
        this.id = id;
        this.isLeaf = isLeaf;
        this.keys = keys;
        this.valuePointers = valuePointers;
        ...
        if (isLeaf) {
            for (let v of valuePointers) {
                if (!(v instanceof Pointer)) {
                    throw new Error('Value pointers must be Pointer objects');
                }
            }
        }
        this.next = next;
...
/**
 * Persistent B+ tree with pointer-based values
 */
export class BPlusTreePtr {
    /**
     * Creates a new persistent B+ tree with pointer-based values
     ...
     */
    _initializeNewTree() {
        const rootNode = new NodeDataPtr(0, true, [], [], [], null);
        this.nextNodeId = 1;
        this._size = 0;
    ...
    _saveMetadata() {
        const metadata = {
            version: 2,  // Version 2 for pointer-based variant
    ...
    _loadMetadata() {
        ...
        if (metadata.version !== 2) {
            throw new Error(`Incompatible tree version: expected 2, got ${metadata.version}`);
        }
    ...
    /**
     * Save a value to disk and return its pointer
     */
    _saveValue(value) {
        const offset = this.file.getFileSize();
        ...
        if (node.isLeaf) {
            for (let i = 0; i < node.keys.length; i++) {
                if (key === node.keys[i]) {
                    ...
                    return this._loadValue(node.valuePointers[i]);
                }
            }
            return undefined;
        } else {
    ...
    _addToNode(node, key, value) {
        if (node.isLeaf) {
            ...
            const valuePointers = [...node.valuePointers];

            const existingIdx = keys.indexOf(key);
            if (existingIdx !== -1) {
                // Update existing key - save new value and update pointer
                const newValuePointer = this._saveValue(value);
                valuePointers[existingIdx] = newValuePointer;
                return {
                    newNode: new NodeDataPtr(node.id, true, keys, valuePointers, [], null)
                };
            }
            ...
            // Save value and store its pointer
            const valuePointer = this._saveValue(value);
            valuePointers.splice(insertIdx, 0, valuePointer);
            ...
            if (keys.length < this.order) {
                ...
                const leftValuePointers = valuePointers.slice(0, mid);
                const rightKeys = keys.slice(mid);
                const rightValuePointers = valuePointers.slice(mid);

                const rightNode = new NodeDataPtr(this.nextNodeId++, true, rightKeys, rightValuePointers, [], null);
                const leftNode = new NodeDataPtr(node.id, true, leftKeys, leftValuePointers, [], null);
            ...
            if (result.newNode) {
                ...
                return {
                    newNode: new NodeDataPtr(node.id, false, keys, [], children, null)
                };
            } else {
                const leftPointer = this._saveNode(result.left);
                const rightPointer = this._saveNode(result.right);
                ...
                if (keys.length < this.order) {
                    ...
                    const leftNode = new NodeDataPtr(node.id, false, leftKeys, [], leftChildren, null);
                    const rightNode = new NodeDataPtr(this.nextNodeId++, false, rightKeys, [], rightChildren, null);
    ...
    _deleteFromNode(node, key) {
        if (node.isLeaf) {
            ...
            const newValuePointers = [...node.valuePointers];
            newKeys.splice(keyIndex, 1);
            newValuePointers.splice(keyIndex, 1);

            return new NodeDataPtr(node.id, true, newKeys, newValuePointers, [], node.next);
        } else {
            let i = 0;
            while (i < node.keys.length && key >= node.keys[i]) {
                i++;
            ...
            return new NodeDataPtr(node.id, false, [...node.keys], [], newChildren, null);
        }
    }
    ...
    /**
     * Async iterator for efficiently traversing all entries
     */
    ...
    *_iterateNode(node) {
        if (node.isLeaf) {
            for (let i = 0; i < node.keys.length; i++) {
                yield {
                    key: node.keys[i],
                    value: this._loadValue(node.valuePointers[i])
                };
            }
        } else {
    ...
    async compact(destSyncHandle) {
        ...
        const newTree = new BPlusTreePtr(destSyncHandle, this.order);
        await newTree.open();
