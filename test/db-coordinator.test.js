/**
 * Coordination-logic tests for src/db-coordinator.js (docs/db-plan.md,
 * "OPFS concurrency"). Runs in plain Node, part of the default `npm test`
 * suite -- no browser needed, because modern Node ships spec-compliant
 * `navigator.locks` and `BroadcastChannel` globals, and connectShared()
 * only actually requires those two plus a storage provider (MemoryStorage
 * Provider here, in place of OPFSStorageProvider). Several connectShared()
 * calls in this same process, sharing one provider by reference, stand in
 * for several tabs sharing one OPFS directory.
 *
