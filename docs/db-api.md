...
(`c/db*.c`, `c/db*.h`) with a thin JS bridge (`src/binjson-wasm.js`,
`src/db.js`, `src/db-coordinator.js`). This document is a complete
reference for the JS-facing API; see `docs/db-plan.md` for the milestone-
by-milestone design history and exact scope decisions behind each feature.
...
See `public/db.html` + `public/db-worker.js` for a live multi-tab demo
(`npm run serve`, then open the page in two or more browser tabs).
...
| `$currentDate` | `{ $currentDate: { lastSeen: true } }` or `{ $type: 'date' }` | Resolved entirely client-side (in `src/binjson-wasm.js`, rewritten into `$set` before crossing into C — only the JS host has a clock). `{$type: 'timestamp'}` isn't supported (no timestamp wire type). |
...
import { OPFSStorageProvider } from './src/db.js';
